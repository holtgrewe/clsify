    ...
    if "-" in ref and "-" in alt:
        ...
        if ref[1] != "-":
            if len(ref) == 1:
                ...
                return "n.%d_%ddel%sins%s" % (pos + 1, pos + len(ref) - 1, only_bases(ref[1:]), alt[1:])
        else:
        ...
        if alt[1] != '-':
            if len(ref) == 1:
                ...
                return "n.%d_%ddel%sins%s" % (pos + 1, pos + len(ref) - 1, ref[1:], only_bases(alt[1:]))
        else:
            if len(ref) == 2:
...
def normalize_var(ref_seq, curr_var, offset):
    ...
    pos = curr_var["pos"] - len(ref) + 1 - offset
    delta = 0
